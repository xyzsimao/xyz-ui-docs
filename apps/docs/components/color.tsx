...
export function Color({
  ...
  function copyHexToClipboard(e: React.MouseEvent) {
    ...
    if (e.shiftKey) {
      ...
      navigator.clipboard.writeText(oklchvalue)
      setCopied('color')
    }
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Button
            ...
            className={cn(
              `bg-${name}-${shade} aspect-square w-full rounded-sm outline -outline-offset-1
               outline-black/10 sm:rounded-md dark:outline-white/10`,
            )}
          />
        </TooltipTrigger>
        <TooltipContent
